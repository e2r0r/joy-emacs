...
(defun get-perl-path ()
  (if (boundp 'regex-tool-perl-path)
	  regex-tool-perl-path
	"perl"))
...
(defun regex-render-perl (regex sample)
  (with-temp-buffer
    (insert (format "my $lines = do {local $/; <DATA>;};
print \"(\";
our $pos=0;
while($lines =~ m/%s/mg) {
  my @groups = @_;
  local $pos = pos($lines) - length($&);
  print \"(\", $pos, \" \", length($&), \" \";
  for $i (1 .. 20) {
      if ($$i){
        print \"(\", $i, \" . \\\"\", $$i, \"\\\") \";
...
__DATA__
%s" regex sample))
   (call-process-region (point-min) (point-max) (get-perl-path) t t)
   (goto-char (point-min))
   (read (current-buffer))))
